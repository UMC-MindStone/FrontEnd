...
class EmotionModel : ViewModel() {
    ...
    fun selectEmotion(recentEmotion: String, intensity: Int, colorResId: Int, isPositive: Boolean) {
        _recentEmotion.value = recentEmotion
        _intensity.value = intensity
        _colorResId.value = colorResId
        _isPositive.value = isPositive

        // 선택된 감정을 상태바 비율에 반영 (Float 변환 추가)
        addEmotion(recentEmotion, intensity.toFloat())
    }
    ...
    fun addEmotion(emotion: String, value: Float) {
        ...
        // ✅ 기존 감정을 유지한 상태에서 비율을 조정 (감정 값 / 전체 합)
        if (total > 0) {
            currentRatios.replaceAll { _, oldValue -> (oldValue / total) }
        }

        Log.d("EmotionModel", "🔍 감정 비율 업데이트: $currentRatios") // 디버깅 추가
        _emotionRatios.value = currentRatios.toMutableMap() // ✅ 변경된 비율 적용
    }
